...
def main():
    ...
    parser.add_argument("--log", default="results.csv", help="Path to a CSV log file (append)")
    ...
    # Auto-detect resume file in template directory if no attachment specified
    ...
    if not attachment_path:
        ...
        for pattern in resume_patterns:
            ...
            if potential_resume.exists():
                ...
                print(f"[INFO] Auto-detected resume: {attachment_path}")
                break
    ...
    with open(csv_path, newline="", encoding="utf-8") as f_in, open(log_path, "a", newline="", encoding="utf-8") as f_log:
        reader = csv.DictReader(f_in)
        fieldnames = ["email", "status", "error", "subject"]
        ...
        for row in reader:
            ...
            if not email:
                writer.writerow({"email": "", "status": "skipped", "error": "missing email", "subject": ""})
                continue

            subject, body = render_templates(subject_tmpl, body_tmpl, row)
            processed += 1
            if args.dry_run:
                print(f"[DRY RUN] Would send to {email!r} | Subject: {subject}")
                writer.writerow({"email": email, "status": "dry_run", "error": "", "subject": subject})
            else:
                try:
                    if args.mode == "smtp":
                    ...
                    writer.writerow({"email": email, "status": "sent", "error": "", "subject": subject})
                    sent += 1
                    if args.rate > 0:
                        time.sleep(args.rate)
                except Exception as e:
                    writer.writerow({"email": email, "status": "failed", "error": str(e), "subject": subject})
                    print(f"[ERROR] {email}: {e}")

            if args.limit and processed >= args.limit:
                break

    print(f"Done. Processed={processed}, Sent={sent}, Log={log_path}")
